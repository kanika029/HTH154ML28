...
class AnomalyClassifier:
    ...
    def classify(self, feature_data: Dict[str, Any], is_stat_anomaly: bool, is_iforest_unusual: bool) -> Dict[str, Any] | None:
        """
        Classifies an anomaly into SPIKE, DRIFT, DROPOUT, STUCK SENSOR.
        Returns detailed anomaly object if anomalous, or None if normal.
        """
        if not is_stat_anomaly:
            if not is_iforest_unusual:
                return None
            # If flagged by multivariate Isolation Forest, ensure this specific sensor has observable deviation
            threshold_roc = self.spike_roc_thresholds.get(feature_data.get("sensor", ""), 2.5)
            ...
            has_sensor_deviation = (
                ...
                or feature_data.get("is_dropout", False)
                or feature_data.get("is_stuck", False)
            )
        ...
        if is_rapid_jump or (is_stat_anomaly and persistence <= 3):
        ...
        if persistence > 3 or baseline_dev > (threshold_roc * 1.5):
